...
<table >
	...
	foreach($arResult["ITEMS"]["AnDelCanBuy"] as $arBasketItems)
	{
		...
		<tr>
			...
			<?endif;?>
			<?if (in_array("QUANTITY", $arParams["COLUMNS_LIST"])):?>
				<td><input maxlength="18" type="text" name="QUANTITY_<?=$arBasketItems["ID"] ?>" value="<?=$arBasketItems["QUANTITY"]?>" size="3" ></td>
			<?endif;?>
			<?if (in_array("DELETE", $arParams["COLUMNS_LIST"])):?>
				<td ><input type="checkbox" name="DELETE_<?=$arBasketItems["ID"] ?>" id="DELETE_<?=$i?>" value="Y"><label for="DELETE_<?=$i?>"></label></td>
			<?endif;?>
			<?/*if (in_array("DELAY", $arParams["COLUMNS_LIST"])):?>
				<td ><input type="checkbox" name="DELAY_<?=$arBasketItems["ID"] ?>" value="Y"></td>
			<?endif;*/?>
			<?if (in_array("WEIGHT", $arParams["COLUMNS_LIST"])):?>
	...
	<tr>
		...
		<?endif;?>
		<?if (in_array("DELETE", $arParams["COLUMNS_LIST"])):?>
			<td ><input type="checkbox" name="DELETE" value="Y" onClick="sale_check_all(this.checked)"></td>
		<?endif;?>
		<?if (in_array("DELAY", $arParams["COLUMNS_LIST"])):?>
			<td>&nbsp;</td>
		<?endif;?>
...
<table width="100%">
	...
	<tr>
		<td width="30%">
			<input class="b-action" type="submit" value="<?echo GetMessage("SALE_REFRESH")?>" name="BasketRefresh"><br />
		...
		<td align="right" width="30%">
			<input class="b-action" type="submit" value="<?echo GetMessage("SALE_ORDER")?>" name="BasketOrder"  id="basketOrderButton2"><br />
