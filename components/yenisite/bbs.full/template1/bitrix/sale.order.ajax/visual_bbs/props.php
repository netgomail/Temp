	...
	if (!empty($arSource))
	{
		foreach($arSource as $arProperties)
		{
			...
			<div class="control-group">
				<label class="control-label" for="<?=$arProperties["FIELD_NAME"]?>"><?= $arProperties["NAME"] ?><?
				...
				:</label>
					...
					if($arProperties["TYPE"] == "CHECKBOX")
					{
						...
						<input class="input-xlarge" type="hidden" name="<?=$arProperties["FIELD_NAME"]?>" value="">
						<input class="input-xlarge" type="checkbox" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" value="Y"<?if ($arProperties["CHECKED"]=="Y") echo " checked";?>>
						<?
					}
					elseif($arProperties["TYPE"] == "TEXT")
					{
						?>
						<input class="input-xlarge" type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>">
						<?
					}
					...
					{
						?>
						<textarea class="input-xlarge" rows="<?=$arProperties["SIZE2"]?>" cols="<?=$arProperties["SIZE1"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>"><?=$arProperties["VALUE"]?></textarea>
						<?
					}
					...
					elseif ($arProperties["TYPE"] == "RADIO")
					{
						...
						{
							?>
							<input class="input-xlarge" type="radio" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>_<?=$arVariants["VALUE"]?>" value="<?=$arVariants["VALUE"]?>"<?if($arVariants["CHECKED"] == "Y") echo " checked";?>> <label for="<?=$arProperties["FIELD_NAME"]?>_<?=$arVariants["VALUE"]?>"><?=$arVariants["NAME"]?></label><br />
							<?
						}
					}
...
<div class="sale_order_table props" id="sale_order_props" <?=($bHideProps && $_POST["showProps"] != "Y")?"style='display:none;'":''?>>
<?
